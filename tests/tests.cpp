...
TEST_F(Tests, negativePositionalIntParams)
{
    int optional = 0;
    parser.addParam(optional, "optional", '1', "Optional integer", OPTIONAL);

    int positional = 0;
    parser.addPositional(positional, "positional", "Positional integer");

    parse({"exe", "-7"});

    ASSERT_EQ(0, optional);
    ASSERT_EQ(-7, positional);
    ...
    ASSERT_EQ(-1, positional);
}

TEST_F(Tests, paramLikePositionalStringParams)
{
    std::string optional;
    parser.addParam(optional, "optional", "Optional string", OPTIONAL);

    std::string positional;
    parser.addPositional(positional, "positional", "Positional string");

    parse({"exe", "--pos"});

    ASSERT_EQ("", optional);
    ASSERT_EQ("--pos", positional);

    parse({"exe", "--positional", "--optional", "--opt"});
